      ...
      <Paper sx={{ height: 500, width: '100%', overflow: 'hidden', minWidth: 650 }}>
        ...
        {loading ? (
          <FontAwesomeIcon
            ...
            size="2x"
            spin
            style={{ marginBlockStart: '24px', width: 'min-content', alignSelf: 'center' }}
          />
        ) : (
          <TableContainer
            className="vertical-scroll-content"
            component={Paper}
            sx={{ maxHeight: 'calc(100% - 80px)', overflow: 'hidden auto' }}
          >
            <Table>
              <TableHead>
                <TableRow sx={{ position: 'sticky', top: 0, backgroundColor: 'white', zIndex: 1 }}>
                  <TableCell>
                    <TableSortLabel active={sortBy === 'name'} direction={sortOrder} onClick={handleSortByName}>
                      Name
                    </TableSortLabel>
                  </TableCell>
